/*
Испралено:
 - fabs(AllPar[j]->Parms[k].Value) < 0.15 сравнение значения с погрешностью по модулю
 - fabs(AllPar[Data[i][k]]->Parms[j].Value/AllPar[Data[i-1][k]]->Parms[j].Value) - выводится значение изменения параметра по модулю для соседних измерений
*/
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
*/
    ...
    for (j=0; j < N; j++)
	  { for (k=0; k < AllPar[j]->n; k++)
	     { if (fabs(AllPar[j]->Parms[k].Value) < 0.15) AllPar[j]->Parms[k].Value = 0.15; }
	  }
	...
	if ((out = fopen(file,"w")) == 0)
     ...
     for (k=0; k < nParm; k++)
	   { fprintf(out,"%s\t\t",Parms[k]);
		 ...
		 for (j=0; j < AllPar[Data[0][k]]->n; j++)
		  {  fprintf(out,"\t %s\t",AllPar[Data[0][k]]->Parms[j].Name);
			 for (i=1; i < nTemp; i++)      fprintf(out," %f\t",fabs(AllPar[Data[i][k]]->Parms[j].Value/AllPar[Data[i-1][k]]->Parms[j].Value)); 
